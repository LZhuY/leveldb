...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* my_batch) {
  ...
  MutexLock l(&mutex_);
  writers_.push_back(&w);///加入写队列
  while (!w.done && &w != writers_.front()) {
    w.cv.Wait(); ///释放锁然后等待唤醒，这时候有可能别的线程在消耗写队列。只有第一个加入写任务的线程在消费队列。
  }
  if (w.done) {
    return w.status;
  }
  ...
  if (status.ok() && my_batch != nullptr) {  // nullptr batch is for compactions
    ...
    // into mem_.
    {
      mutex_.Unlock(); ///释放锁，其他线程可以push数据
  ...
  while (true) { 
    ...
    writers_.pop_front();
    if (ready != &w) { ///w指针在BuildBatchGroup中被改到了writer_.end()
      ready->status = status;
      ready->done = true;
      ready->cv.Signal();///唤醒其他线程
    }
    if (ready == last_writer) break; ///前面那些处理最多到这里，因为中间unlock可能有新的write push 进来
  }

  // Notify new head of write queue
  if (!writers_.empty()) {
    writers_.front()->cv.Signal(); ///唤醒后续线程
  }
...
WriteBatch* DBImpl::BuildBatchGroup(Writer** last_writer) {
  ...
  *last_writer = first;
  ...
  for (; iter != writers_.end(); ++iter) { ///处理writers_中的全部writer
    Writer* w = *iter;
